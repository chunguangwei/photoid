...
```
横向：中线左右各 2.6 个脸宽（总宽 5.2 脸宽，肩宽约 2.5–3 脸宽）
...
- **只裁不补**：ROI 一定钳进源图内。越界补边是构图阶段的语义，提前混进来
  会让坐标系含义不清；
- **收益不足就不裁**：人脸已占源图 28% 以上（本就是证件照/半身照），或
...
| 通道 | 作用范围 | 算法 |
| --- | --- | --- |
| `retouchFace` 美颜 | 人脸椭圆内，按肤色权重加权 | **双尺度**频率分离磨皮 + 匀肤 + 提亮 + 轻度瘦脸 |
| `enhanceClarity` 清晰度 | 整图 | **亮度通道** USM + 局部对比 + S 曲线 + 轻饱和 |
...
| --- | --- | --- |
| 高频 | 毛孔、细纹、噪点 | < 15 |
| 中频 | **色斑、痘印、眼袋、肤色不匀** | 30–60 |

历经两次失败才定型，两次的用户反馈都是「拉满了也看不出效果」：

1. 最早是「整图高斯 × 混合系数」，强度一大就糊五官，只能把上限压到 75%；
2. 改成单尺度高低频分离后，又把「幅度 > 26」一律当作五官结构**全额保留**
   ——恰好把最刺眼的色斑排除在处理之外。

现在拆三层重建：`blurB`（中频以下，脸部立体感）+ 中频细节（按 `keepBlotch`
压制）+ 高频细节（按 `keepDetail` 压制）。要点：

- **中频不能压太狠**（强度满时仍保留 42%）：中频承载脸部立体感，压没了
  就是一张纸片脸；高频可以压到只剩 10%；
- **判定阈值随强度放大**（`tHi = 26 + 40×intensity`）：低强度只碰细腻瑕疵，
  高强度才敢动明显色斑 —— 滑杆因此在全程都有可感知变化；
- 额外加了**提亮**（暗部抬得多、亮部几乎不动）。证件照普遍偏暗，气色的
  改善比单纯磨皮更容易被用户感知。

此外还加了**暖肤**（红通道单独微增 4×intensity）。人对「气色好」的判断
主要来自肤色暖度而不是光滑度，这一项的可感知收益比继续加大磨皮强度高得多，
且不损失任何细节——姊妹项目 ImagePilot 也是同样的结论（它同时做 +5% 提亮
与红通道 +5×i 暖肤）。
...
- 底色 chips 切换 → `recolorWorker` 秒切（**不重跑流水线**）；高精修档
  切换 → 携带新参数重跑；
