...
所以顺序必须是**先人脸、后抠图**：`matteRoiFor()` 用人脸框算出构图框，
外扩 1.45 倍作为 ROI，`cropRgbaWorker` 裁出来再送模型。人像在模型输入里
的占比提升数倍，边缘判别是量级差异。代价是人脸检测与抠图由并行改串行
（人脸检测通常百毫秒级，值得）。
...
- **只裁不补**：ROI 一定钳进源图内。越界补边是构图阶段的语义，提前混进来
  会让坐标系含义不清；
- **收益不足就不裁**：ROI 面积 > 源图 72% 说明本就是半身构图，裁了不改善
  精度，反而多一次全图拷贝，此时返回 null 走原路径。

外扩系数不能小：ROI 若切掉真实人体，掩码会在边界处被硬切，成片上留下一条
直线切痕，比残留色块更难看。
...
| 文件 | 覆盖 |
| --- | --- |
| `matting_framing_test` | **ROI 粗裁契约**（不越界/收益不足不裁）、按框裁像素、掩码精修不腐蚀轮廓、发丝带存活、**中高置信区判实心**（防人物溶解）、**飞地清除/孔洞填实**、真实头顶探测、构图越界补边、消白边不改 alpha 且**单向变暗**（防白色光晕） |
