...
| 页面 | 内容 |
|---|---|
| 启动页 `splash_page` | 与应用图标同构的「Logo 自我组装」动画：取景框四角滑入 → 人像轮廓被一笔描出 → 扫描光带自上而下点亮为实体 → 合规徽章弹入打勾，四拍正好对应产品主流程（取景、识别、处理、通过）。6 秒倒计时可跳过，底部预留广告位槽。这段空闲期同时预热抠图模型，缩短首张照片的等待 |
...
| 处理 `edit_page` | 流水线步骤进度（preparing→…→compressing 六项，配「扫描揭色」动效：照片从灰度逐行变彩色，直观表达处理进度）；五色底色**秒切**；**美颜 / 清晰度双滑杆**（每次进入都从 0 开始）；手势缩放平移的交互裁剪；原图/效果对比切换 |
| 结果 `result_page` | 成片预览 + 合规检测报告（11 项，硬/软分级）+ 保存（应用私有相册，可选写系统相册；文件名默认自动生成，可自定义） |
...
- **五色底色**：蓝 / 白 / 红 / 灰 / 深蓝，全规格通用。
  **默认推荐机制**：规格自带默认底色，详情页对应 chip 标注
  ...
  处理页可随时切换，**换色是秒切**——抠图结果与构图框都与底色无关，
  不需要重新生成，你调好的取景与美颜强度也不会被打回。

## 4. 修图能力

### 4.1 抠图

统一走 MODNet 模型，**没有档位选择**。

早期版本提供过「高精修版 / 原图普通版」切换，但它名不副实：两档的抠图
完全一样（都走 MODNet），唯一差异只是缩放补偿锐化的系数，切换却要重跑
整条流水线（含最贵的模型推理）。v0.7.1 已移除该开关，锐化取固定值。
...
### 4.3 美颜与清晰度（两条独立通道，默认均为 0 且不记忆）

| 滑杆 | 作用对象 | 效果 |
|---|---|---|
| **美颜** | 人脸区域 | 磨掉毛孔细纹与**色斑痘印眼袋**（五官轮廓保持锐利）、匀肤、**提亮与暖肤（改善气色）**，以及幅度 ≤4.5% 的轻度瘦脸 |
...
- 磨皮分两个尺度处理：细腻的毛孔与细纹，以及更大块的色斑、痘印、眼袋和
  肤色不匀——后者才是观感上最刺眼的部分。眼睑、唇线、鼻翼等五官结构靠
  「高对比 + 非肤色」双重保护，拉到 100% 也不会糊。
- 强度全程可感知：拉得越大，敢处理的瑕疵幅度越大，不会出现「拉满了也没变化」。
- 阴影侧和偏暗的肤色同样会被处理（按颜色而非明暗判断皮肤），侧脸、
  逆光照不会漏掉。
- 清晰度的锐化只作用于明暗层次，不动色彩，所以不会出现发丝、眼镜框上的
  彩色描边，暗部也不会被压成死黑；配套的「鲜艳度」只提升灰淡区域，
  对本就饱和的肤色自动收手，不会把人调成橙色。
