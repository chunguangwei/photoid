...
| 处理 `edit_page` | 流水线步骤进度（preparing→…→compressing 六项）；高精修/普通档切换；五色底色切换即重跑；**美颜 / 清晰度双滑杆**；手势缩放平移的交互裁剪；原图/效果对比切换 |
...
- 瘦脸幅度刻意做得很轻——证件照审核关注「与本人一致」，明显改脸型有
  合规风险。
- 调节时预览区叠加扫描动效表明正在处理；松手后自动重算，最终成片与
  预览走完全相同的参数与算法。
- 强度持久化，下次进处理页沿用上次的选择。
